...
<head>
    ...
    <title>hachiroku - Регистрация</title>
        ...
        <header class="flex flex-wrap justify-between items-center gap-4 py-8 sm:py-10 mb-8 sm:mb-12">
            ...
            <nav class="flex items-center gap-4 sm:gap-6 md:gap-11">
                <ul class="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 md:gap-11 text-lg sm:text-xl md:text-2xl text-white">
                    ...
                    @else
                        <li><a href="{{ route('login') }}" class="font-rubik-light hover:opacity-80 transition">Вход</a></li>
                        <li><a href="{{ route('register') }}" class="font-rubik-light hover:opacity-80 transition text-pink-200">Регистрация</a></li> @endauth
                </ul>
            </nav>
        </header>
    </div>
    ...
    <main class="flex-grow flex flex-col items-center justify-center py-10 px-4 sm:px-6 lg:px-8">
        <div class="w-full max-w-lg bg-slate-800/30 p-8 sm:p-10 rounded-xl shadow-xl">
            <div class="text-center mb-8">
                <h1 class="font-rubik-semibold text-3xl sm:text-4xl text-pink-200">Регистрация</h1>
            </div>
            ...
            <form method="POST" action="{{ route('register') }}">
                ...
                <div>
                    <x-input-label for="name" :value="__('Имя')" class="block font-rubik-regular text-lg text-white/80 mb-1" />
                    <x-text-input id="name" class="block mt-1 w-full bg-white/10 border border-white/30 text-white font-rubik-light p-3 rounded-md focus:border-pink-200 focus:ring focus:ring-pink-200 focus:ring-opacity-50 placeholder-white/50" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
                    <x-input-error :messages="$errors->get('name')" class="mt-1 text-sm text-red-400 font-rubik-light" />
                </div>

                <div class="mt-6">
                    <x-input-label for="email" :value="__('Email')" class="block font-rubik-regular text-lg text-white/80 mb-1" />
                    <x-text-input id="email" class="block mt-1 w-full bg-white/10 border border-white/30 text-white font-rubik-light p-3 rounded-md focus:border-pink-200 focus:ring focus:ring-pink-200 focus:ring-opacity-50 placeholder-white/50" type="email" name="email" :value="old('email')" required autocomplete="username" />
                    <x-input-error :messages="$errors->get('email')" class="mt-1 text-sm text-red-400 font-rubik-light" />
                </div>

                <div class="mt-6">
                    <label for="phone" class="block font-rubik-regular text-lg text-white/80 mb-1">Телефон</label>
                    <input id="phone" class="block mt-1 w-full bg-white/10 border border-white/30 text-white font-rubik-light p-3 rounded-md focus:border-pink-200 focus:ring focus:ring-pink-200 focus:ring-opacity-50 placeholder-white/50" type="text" name="phone" :value="old('phone')" required autocomplete="tel">
                    <x-input-error :messages="$errors->get('phone')" class="mt-1 text-sm text-red-400 font-rubik-light" />
                </div>
                ...
                <div class="mt-6">
                    <x-input-label for="password_confirmation" :value="__('Подтвердите пароль')" class="block font-rubik-regular text-lg text-white/80 mb-1" />
                    <x-text-input id="password_confirmation" class="block mt-1 w-full bg-white/10 border border-white/30 text-white font-rubik-light p-3 rounded-md focus:border-pink-200 focus:ring focus:ring-pink-200 focus:ring-opacity-50 placeholder-white/50"
                                    type="password"
                                    name="password_confirmation" required autocomplete="new-password" />
                    <x-input-error :messages="$errors->get('password_confirmation')" class="mt-1 text-sm text-red-400 font-rubik-light" />
                </div>

                <div class="flex flex-col sm:flex-row items-center justify-between mt-8">
                    <a class="font-rubik-light text-sm text-white/70 hover:text-pink-200 hover:underline focus:outline-none focus:ring-2 focus:ring-pink-200/50 rounded-md mb-4 sm:mb-0" href="{{ route('login') }}">
